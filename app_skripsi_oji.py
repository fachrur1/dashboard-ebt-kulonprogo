        ...
        with tab1:
            ...
            with col_c2:
                ...
                if not data_tahun_terakhir.empty:
                    ...
                    st.warning("Data bulanan tidak tersedia untuk tahun yang dipilih.")
...
Data: Tahun {tahun_evaluasi}, Skenario {kebijakan}, Inflasi {inflasi*100:.1f}%. Ranking: {hasil_df.head(3).to_string()}. Jawab HANYA JSON."""
                        ...
                        st.session_state.ai_result_text = {
                            ...
                            "dampak_ekonomi": "Silakan coba lagi dengan API Key yang valid.",
                            "rekomendasi": "Periksa koneksi internet dan kredensial API."
                        }
        ...
        with tab3:
                ...
                for _, row in df_lokasi.iterrows():
                    fig_3d.add_trace(go.Scatter3d(
                        ...
                        hovertemplate=f"<b>{row['Teknologi']}</b><br>" +
                                     f"Lokasi: {row['Kecamatan']}<br>" +
                                     f"Potensi: {row['Potensi_MW']} MW<br>" +
                                     f"Investasi: Rp {row['Investasi_M_IDR']} M<br>" +
                                     f"Elevasi: {row['Elevasi']} m"
                    ))
                fig_3d.update_layout(
                    scene=dict(
                        bgcolor="rgb(10,15,30)",
                        xaxis=dict(gridcolor="#1e3a5f", title="Longitude"),
            ...
            fig_tl.update_layout(xaxis_title="Tahun", yaxis_title="Teknologi")
        ...
        with st.expander("🔍 Lihat detail error"):
            st.exception(e)
        st.info("💡 Pastikan file yang diupload memiliki format yang benar dan mengandung kolom 'Tahun' serta parameter EBT yang valid.")

else:
    st.info("👈 Upload file JSON/CSV NASA POWER atau Excel di sidebar untuk memulai.")
    st.markdown("""
    ### 📖 Panduan Penggunaan:
    1. **Upload Data**: Gunakan file dari NASA POWER (CSV/JSON) atau buat file Excel dengan kolom 'Tahun' dan parameter EBT
    2. **Atur Kebijakan**: Pilih skenario kebijakan dan sesuaikan parameter inflasi
