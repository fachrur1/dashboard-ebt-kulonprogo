...
# ==========================================
# SESSION STATE (Menyimpan AI Insight agar tidak hilang)
# ==========================================
...
if uploaded_file is not None:
    try:
        ...
        with tab1:
            ...
            fig_tren.add_vline(x=2025, line_dash='dash', line_color='red')
            ...
            with col_c1:
                ...
                fig_bar = px.bar(
                    ...
                    x="Periode", y="MW", color="Teknologi", barmode="group", template="plotly_dark"
                )
            ...
            with col_c2:
                ...
                if not data_tahun_terakhir.empty:
                    ...
                    fig_line = px.line(
                        ...
                        x="index", y="MW", color="Teknologi", template="plotly_dark"
                    )
        ...
        with tab2:
            ...
            with col_r1:
                fig_rank = px.bar(
                    ...
                    x="Skor Preferensi", y="Teknologi", orientation="h", color="Skor Preferensi",
            ...
            with col_r2:
                ...
                for alt in weighted_df.index:
                    ...
                    fig_radar.add_trace(go.Scatterpolar(
                        r=vals + [vals[0]],
                        theta=[f"{d} ({bobot_label[d]*100:.0f}%)" for d in categories] + [f"{categories[0]} ({bobot_label[categories[0]]*100:.0f}%)"],
                        fill='toself', name=alt
                    ))
                fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, max(w_tech, w_econ, w_env, w_soc) * 1.1])), template="plotly_dark", height=320)
...
Data: Tahun {tahun_evaluasi}, Skenario {kebijakan}, Inflasi {inflasi*100:.1f}%. Ranking: {hasil_df.to_string()}. Jawab HANYA JSON."""
                    ...
                    except Exception as e: st.error(f"Kesalahan AI: {e}")
            ...
            if st.session_state.ai_chart_data is not None:
                ...
                fig_sc = px.bar(st.session_state.ai_chart_data, x="Teknologi", y="Skor", color="Skenario", barmode="group", template="plotly_dark")
        ...
        with tab3:
            ...
            if mode_peta == "🗺️ 2D Interaktif":
                ...
                for _, row in df_lokasi.iterrows():
                    popup_html = f"<b>{row['Teknologi']}</b><br>📍 {row['Kecamatan']}<br>⚡ {row['Potensi_MW']} MW"
                ...
                for _, row in df_lokasi.iterrows():
                    fig_3d.add_trace(go.Scatter3d(
                        x=[row['Lon']], y=[row['Lat']], z=[row['Elevasi']], mode='markers+text',
                        marker=dict(size=row['Potensi_MW'] * 0.8, color=color_map.get(row['Teknologi'], "#fff"), opacity=0.85),
                        text=[f"{row['Teknologi']}<br>{row['Potensi_MW']} MW"], name=row['Teknologi']
                    ))
                fig_3d.update_layout(scene=dict(bgcolor="rgb(10,15,30)", xaxis=dict(gridcolor="#1e3a5f"), yaxis=dict(gridcolor="#1e3a5f"), zaxis=dict(gridcolor="#1e3a5f")), template="plotly_dark", height=520)
            ...
            fig_tl = px.timeline(
                df_tl.assign(Mulai=pd.to_datetime(df_tl["Mulai"].astype(str) + "-01-01"), Selesai=pd.to_datetime(df_tl["Selesai"].astype(str) + "-12-31")),
                x_start="Mulai", x_end="Selesai", y="Teknologi", color="APBN", template="plotly_dark"
            )
